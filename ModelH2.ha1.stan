//  Model H2.ha1:  trust covariates but no new variant or vaccine  
...
transformed parameters {
  ...
  for (n in 1:N){
   ...
   mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha1_primed[n] +d0*lag1hcw_primed[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+vol*trustvolperbed_std[n] + sizetrust0*trustsize_std[n]) ;
   phi[n] = lag1hcw[n];
  }
}
