// Model ADM3.ha2:  dependent var is covid confirmed admissions,  adjusting for other infecteds
...
transformed parameters {
  ...
  //    mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha2_primed[n] +d0*lag1hcw_primed[n]*exp(v0*hcw_vax_plus3[n]))*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ vol*trustvolperbed_std[n]+ nv0*prop_nv[n]) ;
      mu[n] = (a[trustindex[n]] + b0*lag1adm_primed[n] + c0*lag1ha2_primed[n] +d0*lag1hcw_primed[n]*exp(v0*hcw_vax_plus3[n]))*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ vol*trustvolperbed_std[n]+ nv0*prop_nv[n]) ;
