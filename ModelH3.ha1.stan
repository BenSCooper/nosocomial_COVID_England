//  Model H3.ha1:  trust covariates and new variant plus vaccine  
...
transformed parameters {
  ...
  for (n in 1:N){
   ...
   mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha1_primed[n] +d0*lag1hcw_primed[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+vol*trustvolperbed_std[n] + sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]+v0*hcw_vax_plus3[n]) ;
   phi[n] = lag1hcw[n];
  }
}
