...
transformed parameters {
 ...
 //   mu[n] = (a[trustindex[n]] + b0*lag1adm_primed[n] + c0*lag1ha2_primed[n] +d0*lag1hcw_primed[n]) ;
  //  mu[n] = (a[trustindex[n]] + b0*lag1adm_primed[n] + c0*lag1ha2_primed[n]) ;
      mu[n] = (a[trustindex[n]] + b0*lag1adm_primed[n] + c0*lag1ha2_primed[n] +d0*lag1hcw_primed[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ vol*trustvolperbed_std[n]) ;
