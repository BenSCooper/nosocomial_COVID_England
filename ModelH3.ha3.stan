//  Model H3.ha3:  trust covariates and new variant plus vaccine  
...
transformed parameters {
  ...
  for (n in 1:N){
   ...
   mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha3_primed[n] +d0*lag1hcw_primed[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n] +vol*trustvolperbed_std[n] + sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]+v0*hcw_vax_plus3[n]) ;
   phi[n] = lag1hcw[n];
  }
}
...
// contribution to mean from community acquired cases
  for (n in 1:N) ca_totals[n] = b0*lag1_ca_primed[n]*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]+v0*hcw_vax_plus3[n]); 
  
 // contribution to mean from hospital acquired cases
  for (n in 1:N) ha_totals[n] = c0*lag1ha3_primed[n]*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]) ;
  
   // contribution to mean from hcws
  //for (n in 1:N) hcw_totals[n] = d0*lag1hcw_primed[n]*exp(v0*hcw_vax_plus3[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]) ; 
  for (n in 1:N) hcw_totals[n] = d0*lag1hcw_primed[n]*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ nv0*prop_nv[n]) ; 
