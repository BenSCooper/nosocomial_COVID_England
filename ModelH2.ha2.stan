//  Model H2.ha2:  trust covariates but no new variant or vaccine  
...
transformed data{
  ...
  for (n in 1:N){
   ...
   lag1ha2_primed[n]  = lag1ha2[n]/15.2;
...
transformed parameters {
  ...
  for (n in 1:N){
   ...
   mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha2_primed[n] +d0*lag1hcw_primed[n])*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+vol*trustvolperbed_std[n] + sizetrust0*trustsize_std[n]) ;
   phi[n] = lag1hcw[n];
  }
}
