...
transformed parameters {
  ...
  for (n in 1:N){
   ...
   mu[n] = (a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha3_primed[n] +d0*lag1hcw_primed[n]*exp(v0*hcw_vax_plus3[n]))*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n] + vol*trustvolperbed_std[n] + nv0*prop_nv[n]) ;
   phi[n] = lag1ha3[n];
  }
}
...
//  sigmasq_k ~  student_t(3,0,1); 
 ...
 // sigmasq_a ~ student_t(3,0,1); //slightly heavier tailed prior 
  ...
  for (i in 1:N){
    ha3[i] ~ neg_binomial_2(mu[i], phi0 + k[trustindex[i]]* phi[i]);
  }  
}
...
generated quantities {
 ...
 // contribution to mean from hospital acquired cases
  for (n in 1:N) ha_totals[n] = c0*lag1ha3_primed[n]*exp(sr0*proportion_single_rooms_std[n]+occ0*occupancy_std[n]+agetrust0*percent_pre1965_std[n]+ sizetrust0*trustsize_std[n]+ vol*trustvolperbed_std[n]+ nv0*prop_nv[n]) ;
