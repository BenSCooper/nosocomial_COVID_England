//  Model P1:  covariates but no new variant or vaccine  
  data {
    ...
    int<lower=0> lag2hcw[N];    // hcws infected (imputed) in previous week 
  }
  
transformed data{
  
//  real<lower=0> lag1hcw_primed[N];
//  real<lower=0> lag1ha1_primed[N];
//  real<lower=0> lag1_ca_primed[N];

//  for (n in 1:N){
//   lag1hcw_primed[n] = lag1hcw[n]/124.3; 
//   lag1ha1_primed[n]  = lag1ha1[n]/25.2;
...
//    a[i] = a0 + a_raw[i]*sigmasq_a;
  ...
  for (n in 1:N){
   ...
   //this has no mixing problems  
   mu[n] = a0 + b0*lag1_ca[n] + c0*lag1ha1[n] +d0*lag1hcw[n] ;
   //mu[n] = a[trustindex[n]] + b0*lag1_ca_primed[n] + c0*lag1ha1_primed[n] +d0*lag1hcw_primed[n] ;
   phi[n] = lag1ha1[n];
  }
...
//  a_raw ~ normal(0, 1);   //for non-centred parameterisation
 ...
 // sigmasq_a ~ student_t(3,0,1); //slightly heavier tailed prior 
  ...
  for (i in 1:N){
    ha1[i] ~ neg_binomial_2(mu[i], phi0 + k[trustindex[i]]* phi[i]);
  }  
}
...
// real hcw_coeff;
 ...
 for (n in 1:N) log_lik[n] = neg_binomial_2_lpmf( ha1[n]| mu[n],  phi0 + k[trustindex[n]]*phi[n]); 
